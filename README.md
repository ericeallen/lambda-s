...
| `sorry` / `admit` | none |
| theorems and lemmas | 273 |
| axioms | `propext`, `Classical.choice`, `Quot.sound` |

`Examples.lean`, `QM.lean`, and `Algorithms.lean` run the checker and the
evaluator **at build time** through `#guard`. If any stated result were
different, the library would not compile.
...
lake exe lambdas       # run the worked declaration examples
```

`lake exe cache get` is the slow step and needs network. The BLAS shim in
`c/` links against Accelerate on macOS and falls back to portable C loops
elsewhere; no external library is required.
...
units. `Soundness` is type soundness, `Normalization` is strong
...
as a corollary of parametricity. `Definability` and `NonDefinability` prove
that roots must be primitive.
...
exists; those are the intended entry points for a reader.

## Theorem index

`THEOREMS.md` maps every theorem stated in the accompanying paper to its Lean
name, file, and line.
...
prints the axiom dependencies of the headline theorems. CI runs this and
fails if `sorryAx` appears anywhere.
