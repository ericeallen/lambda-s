...
summarizes.
 ...
 sectionMechanization]


The development is roughly fourteen thousand lines of Lean 4 [cite: lean4],
building with no  textsfsorry] (Lean's placeholder for an unproved
obligation) and no axioms beyond the three of Lean's
standard library. [footnote: Propositional extensionality, choice, and quotient
...
examples of Sections [ref: sec:declarations]--[ref: sec:hart] run as
...
beyond the kernel.
 ...
 paragraphExponent vectors make the metatheory linear algebra.]
The decision of Section [ref: sec:units] propagates through every file.
...
It is a Fubini argument: an interchange of the order of a finite double sum. In his Coq development, [cite: kennedy2008] reports the
...
ground type) rather than by a metalanguage type, never needs such coercions. Where [cite: fosterwolff2023] implement dimension normalization in ML
because their host type system cannot reduce
L] cdotT]^-1] cdotT],
our equality is **definitional]: no datatype of unit expressions exists
anywhere in the mechanization (Section [ref: sec:units]), so a unit
expression is its own normal form.

 paragraphDerivations as data collapse the trusted base.]
Making the typing judgment Type]-valued and the checker
derivation-returning (Section [ref: sec:statics]) follows the
intrinsically-typed tradition [cite: altenkirchreus1999,poulsen2018]; the technique is
...
Theorem [ref: thm:completeness], and with it the right to define semantics
by recursion on derivations while stating side conditions on terms.

 paragraphAn environment-passing normalizer avoids Kripke structure.]
The ratio normalizer of Section [ref: sec:twist] must interpret a binder
...
order of summation.
 ...
 paragraphWhat the composition laws caught.]
...
preserved in the artifact as a regression test.
 ...
 paragraphNumbers, twice.]
The semantic carrier is a type class with two instances. At ℝ it
is noncomputable, and it is the object of the theorems. At  textsfFloat]
it is compiled, calling BLAS through Lean's FFI where the platform supplies
it, on unboxed arrays; Section [ref: sec:hart]'s rank-one structure is why
...
(adequacy, Theorem [ref: thm:factors-reach-machine], drift independence)
...
checked at build
time, in exact arithmetic where the declarations live and in  textsfFloat]
where the binary computes; the FFI-reaching reports are checked by the
binary itself when it runs.

 paragraphThe carrier boundary.]
The two instances agree, up to rounding and
overflow, wherever the classical operations are defined, and at negative
bases of the power primitive, where the  textsfFloat] instance computes
the semantics' principal-power value |x|^q] cos(q pi) rather than C's
NaN: that totalization is the extension the covariance
identity of Section [ref: sec:semantics] is proved for. Where the classical
...
serve the proofs, and the  textsfFloat] side follows IEEE's specials,
...
standard [cite: ieee754-2019] requires correct rounding of the field
operations but only recommends it for  textttpow],  log, and  exp,
so conformance pins down nothing about the accuracy of exactly the
operations at issue.

 paragraphThe trusted base, enumerated.]
A reader who believes a theorem of this paper trusts the Lean kernel and
...
binary prints trusts, in addition: Lean's code generator and runtime;
 textsfFloat] arithmetic, whose operations are opaque primitives with no
