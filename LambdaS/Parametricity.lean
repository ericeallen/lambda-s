...
* `relQ_rpow` carries **no sign hypothesis on the value**, and that is a
  ...
  all (`(-8) ^ (1/3 : ℝ)` denotes `1` under `rpow`; the `Float` carrier
  returns `NaN` there, an undefined point of the classical operation like
  division by zero, see `LambdaS.Num`), so the lemma certifies covariance of
  Mathlib's total extension, not root-ness.
